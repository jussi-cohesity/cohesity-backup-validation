...
task doValidationTests {
    ...
    foreach ($clone in $clones) {
        ...
        $vmCredentials = Import-Clixml -Path ($Config.virtualMachines[$i].guestCred)
        Invoke-Build -File .\validationTests.ps1 -Tasks $Config.virtualMachines[$i].tasks -Config $Config.virtualMachines[$i] -vmCredentials $vmCredentials -vmName $($Clone.Name)
